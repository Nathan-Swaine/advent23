...
def parse(lines):
    ...
    for line in lines:
        ...
        velocity.append(Vec3(*map(int, v.split(","))))
    return (pts, velocity)

def find(pts, velocity):
    n = len(pts)

    p1, v1 = pts[0], velocity[0]
    for i in range(1, n):
        if indep(v1, velocity[i]):
            p2, v2 = pts[i], velocity[i]
            break
    for j in range(i+1, n):
        if indep(v1, velocity[j]) and indep(v2, velocity[j]):
            p3, v3 = pts[j], velocity[j]
            break
